...
#ifdef CONFIG_NETWORK_TLS_WOLFSSL
...
handle_t tls_connect(const uint8_t *host, uint16_t port, const uint8_t *ca_cert, uint16_t ca_cert_len, uint32_t timeout)
{
    ...
    if (SSL_SUCCESS != wolfSSL_CTX_load_verify_buffer(net->wolf_ctx, ca_cert, ca_cert_len, WOLFSSL_FILETYPE_PEM))
    {
        goto exit1;
    }
    ...
    if (0 > (net->handle = plat_tcp_connect(host, port, countdown_left(tmr))))
    {
        goto exit1;
    }
