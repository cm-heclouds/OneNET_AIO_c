...
{
    {.data_id="3311_0_5850", .data_type=IOT_DATA_TYPE_INT,      .buf.val_int=0      },
    {.data_id="3311_0_5851", .data_type=IOT_DATA_TYPE_INT,      .buf.val_int=60     },
    {.data_id="3311_1_5850", .data_type=IOT_DATA_TYPE_INT,      .buf.val_int=1      },
    {.data_id="3311_1_5851", .data_type=IOT_DATA_TYPE_INT,      .buf.val_int=80     },
    {.data_id="3303_0_5700", .data_type=IOT_DATA_TYPE_FLOAT,    .buf.val_float=57.12},
    {.data_id="3303_0_5701", .data_type=IOT_DATA_TYPE_STRING,   .buf.val_str="sheshidu"},
};
...
static int32_t iot_dev_m2m_write(struct m2m_data_t *data_info, struct iot_data_t* data)
{
    ...
    switch(data_info->data_type)
    {
        ...
        case M2M_DT_STRING:
            ...
            osl_memcpy(data->buf.val_str, data_info->data.val_str,
                       osl_strlen(data_info->data.val_str));
            break;
        case M2M_DT_OPAQUE:
            osl_memset(data->buf.val_bin.buf, 0, data->buf.val_bin.buf_len);
            data->buf.val_bin.buf_len = data_info->data.val_opa.data_len;
            osl_memcpy(data->buf.val_bin.buf, data_info->data.val_opa.data, 
                       data->buf.val_bin.buf_len);
            break;
        default:
            ret = 11;
            break;
...
int32_t nbiot_cb_func(enum iot_dev_event_e event, void *data, uint32_t data_len)
{
    switch(event)
    {
        ...
        case IOT_DEV_CMD_RECEIVED:
        {
            ...
            osl_sprintf(&data_id[0], "%d_%d_%d", m2m_attr_ptr->data_info.res_info.obj_id,
                                                 m2m_attr_ptr->data_info.res_info.inst_id,
                                                 m2m_attr_ptr->data_info.res_info.res_id);
