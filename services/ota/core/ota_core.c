...
uint8_t ota_loop(void *ota_ctx, uint32_t timeout_ms)
{
    ...
    case CTX_STATE_CHECK_TASK: {
        if(ctx->check_timer.check_task_period == 0 || ctx->check_timer.last_check_time == 0 ||
           ota_get_tick_ms() - ctx->check_timer.last_check_time >=
               ctx->check_timer.check_task_period * 1000)
        {
    ...
    case CTX_STATE_VALIDATE_TOKEN: {
        if(OTA_Validate_Token(ctx, timeout_ms) == OTA_OK)
        {
    ...
    case CTX_STATE_DOWNLOAD_PACKAGE: {
        ...
        if(OTA_Download_Package(ctx, _start_, _end_, timeout_ms) == OTA_OK)
        {
            ...
            {
                ctx->state = CTX_STATE_VALIDATE_TOKEN;
            }
            return OTA_OK;
        }
        return OTA_ERROR;
...
uint8_t OTA_Event_Handle(void *context, ota_event_code_t EVENT)
{
    ...
    case OTA_EVENT_SUBMIT_VERSION_ERROR: {
        ota_log_printf("Submit Version Error\r\n");
        break;
    }
    case OTA_EVENT_CHECK_TASK_SUCCESS: {
        ctx->state = CTX_STATE_VALIDATE_TOKEN;
        ota_log_printf("Check OTATask Success.\r\n");

        ota_scene_store(ctx);

        OTA_Event_Handle(ctx, OTA_EVENT_custom_delete_package);

        //if battery is below ctx->task_info.power
        /*
        ctx->state = CTX_STATE_REPORT;
        ctx->report_code = OTA_EVENT_REPORT_DOWNLOAD_LOW_POWER_ERROR;
        */
        //if signal strength is below ctx->task_info.signal
        /*
        ctx->state = CTX_STATE_REPORT;
        ctx->report_code = OTA_EVENT_REPORT_DOWNLOAD_BAD_SIGNAL_ERROR;
        */
        break;
    }
    ...
    case OTA_EVENT_TOKEN_OK: {
        ota_log_printf("\r\nCheck OTA TOKEN Success.\r\n");
        break;
    }
    case OTA_EVENT_TOKEN_TASK_EXPIRED_ERROR: {
        ota_log_printf("ERROR CODE: %d. OTATask Was Expired.\r\n", TOKEN_CHECK_Task_Expired);
        ota_log_printf("Destroy OTA Context\r\n");
        ctx->state = CTX_STATE_FINISH_UPDATE;
        break;
    }
    case OTA_EVENT_TOKEN_OTHER_ERROR: {
        ota_log_printf("Check TOKEN Other Error. Body: %.*s\n", ctx->net.HTTP_body_length,
                       (char *)(ctx->net.HTTP_body_start_ptr));
        ota_log_printf("Destroy OTA Context\r\n");
        ctx->state = CTX_STATE_FINISH_UPDATE;
        break;
    ...
    case OTA_EVENT_custom_ready_update: {
        ...
        else
        {
            ...
            ctx->state = CTX_STATE_REPORT;
            ctx->report_code = OTA_EVENT_REPORT_UPDATE_SUCCESS;
            if(ctx->task_info.type == 2)
            {
                ota_free(ctx->device_info.s_version);
                ctx->device_info.s_version_len = ota_strlen(ctx->task_info.target) + 1;
                ctx->device_info.s_version = ota_malloc(ctx->device_info.s_version_len);
                ...
                ctx->device_info.s_version_len = ota_strlen(ctx->device_info.s_version);
            }
        }
        fclose(fsread);
        break;
