...
uint8_t otaParse_Check_Task(ota_context *ctx)
{
    ...
    // token
    item = cJSON_GetObjectItem(root_data, "token");
    if(ota_copy_json_str_item(item, &ctx->task_info.token) == OTA_ERROR)
    {
        goto _parse_check_task_error_;
    }
    // size
    ...
    // signal
    item = cJSON_GetObjectItem(root_data, "signal");
    if(item == NULL)
    {
        goto _parse_check_task_error_;
    }
    ctx->task_info.signal = item->valueint;
    // power
    item = cJSON_GetObjectItem(root_data, "power");
    if(item == NULL)
    {
        goto _parse_check_task_error_;
    }
    ctx->task_info.power = item->valueint;
    // retry
    item = cJSON_GetObjectItem(root_data, "retry");
    if(item == NULL)
    {
        goto _parse_check_task_error_;
    }
    ctx->task_info.retry = item->valueint;
    // interval
    item = cJSON_GetObjectItem(root_data, "interval");
    if(item == NULL)
    {
        goto _parse_check_task_error_;
    }
    ctx->task_info.interval = item->valueint;
...
uint8_t otaParse_Validate_Token(ota_context *ctx)
{
    const char *buf = ctx->net.HTTP_body_start_ptr;
    ...
    if(item != NULL)
    {
        ...
        case TOKEN_CHECK_Task_Expired: {
            OTA_Event_Handle(ctx, OTA_EVENT_TOKEN_TASK_EXPIRED_ERROR);
            break;
        }
        default: {
            OTA_Event_Handle(ctx, OTA_EVENT_TOKEN_OTHER_ERROR);
            break;
        }
        }
    }
...
uint8_t otaParse_Download_Package(ota_context *ctx)
{
    ...
    {
        ota_log_printf("range doesn't match.%d-%d | %d-%d\n", dl_info->range_next_start,
                       dl_info->range_next_end, dl_info->content_range_start,
                       dl_info->content_range_end);
