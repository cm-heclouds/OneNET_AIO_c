...
int32_t iot_nbiot_step(void *dev, uint32_t timeout_ms)
{
    ...
    switch(nbiot_dev->state)
    {
        ...
        {
            ret = m2m_register(nbiot_dev->m2m_ptr, 
                               nbiot_dev->cloud_info->auth_code, 
                               nbiot_dev->cloud_info->life_time, timeout_ms);
            if(ret == ERR_OK)
            {
...
int32_t iot_nbiot_push_rawdata(void *dev, const int8_t *target, void *raw_data, uint32_t data_len, uint32_t timeout_ms)
{
    struct iot_nbiot_t *nbiot_dev = (struct iot_nbiot_t *)dev;
    struct m2m_data_t m2m_data;

    osl_sscanf(target, "%d_%d_%d", &m2m_data.res_info.obj_id, 
                                   &m2m_data.res_info.inst_id, 
...
int32_t iot_nbiot_notify(void *dev, struct iot_data_t *data, uint32_t timeout_ms)
{
    ...
    osl_sscanf(data->data_id, "%d_%d_%d", &m2m_data.res_info.obj_id, 
                                          &m2m_data.res_info.inst_id,
                                          &m2m_data.res_info.res_id);
...
int32_t iot_nbiot_observe(void *dev, const int8_t *resource, uint32_t timeout_ms)
{
    int32_t ret = ERR_OTHERS;
    struct m2m_res_info_t res_info = {0};
    struct iot_nbiot_t *nbiot_dev = (struct iot_nbiot_t *)dev;

    osl_sscanf(resource, "%d_%d_%d", &res_info.obj_id, 
                                     &res_info.inst_id, 
                                     &res_info.res_id);
...
int32_t iot_nbiot_observe_cancel(void *dev, const int8_t *resource, uint32_t timeout_ms)
{
    struct m2m_res_info_t res_info = {0};
    struct iot_nbiot_t *nbiot_dev = (struct iot_nbiot_t *)dev;
