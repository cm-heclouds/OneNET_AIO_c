...
int32_t m2m_register(void *arg, int8_t *auth_code, uint32_t life_time, uint32_t timeout_ms)
{
    int32_t i, j = 0;
...
int32_t m2m_read_reply(uint32_t msg_id, uint32_t result_code, struct m2m_data_t *data, uint32_t timeout_ms)
{
    ...
    m5311_mipl_read_resp(msg_id, result_code, data, timeout_ms);
#endif
}
