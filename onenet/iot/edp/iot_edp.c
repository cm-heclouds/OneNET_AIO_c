...
static int32_t process_cmd(struct iot_edp_t *dev, uint8_t *buf, handle_t cd_tmr)
{
    ...
    if(ERR_OK == dev->evt_cb(IOT_DEV_CMD_RECEIVED, &dev_cmd, sizeof(dev_cmd)))
    {
        ...
        send_len = offset;
        if(send_len != tcp_send(dev->conn_fd, dev->send_buf, offset, countdown_left(cd_tmr)))
            ret = ERR_NETWORK;
        else
...
static int32_t edp_parse_data(struct iot_edp_t *dev, uint8_t *msg_type, handle_t cd_tmr)
{
    ...
    recv_len = tcp_recv(dev->conn_fd, dev->recv_buf, 1, countdown_left(cd_tmr));
    if(0 == recv_len)
        return ERR_OK;
    if(0 > recv_len)
        return ERR_NETWORK;
    ...
    *msg_type = *dev->recv_buf >> 4;
    ...
    if(remain_len)
    {
        recv_len = tcp_recv(dev->conn_fd, dev->recv_buf, remain_len, countdown_left(cd_tmr));
...
static uint32_t pack_payload_login(uint8_t *buf, const int8_t *product_id, const int8_t *device_sn, uint32_t life_time)
{
    uint32_t offset = 0;
...
static uint32_t pack_payload_push_data(uint8_t *buf, const int8_t *target, uint8_t *data, uint32_t data_len)
{
    uint32_t offset = 0;
...
static uint32_t construct_login(uint8_t *buf, const int8_t *product_id, const int8_t *device_sn, uint32_t life_time)
{
    uint32_t remain_len = 0;
...
static uint32_t construct_push_data(uint8_t *buf, const int8_t *target, uint8_t *data, uint32_t data_len)
{
    uint32_t remain_len = 0;
    uint8_t *remain_data = buf + EDP_REMAIN_LENGTH_MAX_BYTES;
...
static int32_t edp_login(struct iot_edp_t *dev, const int8_t *product_id, const int8_t *device_sn, uint32_t life_time,
                         handle_t cd_tmr)
{
    uint32_t send_len = 0;
    ...
    dev->conn_fd = tcp_connect(IOT_EDP_SERVER_ADDR, IOT_EDP_SERVER_PORT, countdown_left(cd_tmr));

    if(dev->conn_fd < 0)
        return ERR_NETWORK;

    send_len = construct_login(dev->send_buf, product_id, device_sn, life_time);
    if(send_len != tcp_send(dev->conn_fd, dev->send_buf, send_len, countdown_left(cd_tmr)))
    {
        tcp_disconnect(dev->conn_fd);
        return ERR_NETWORK;
    }
    dev->last_send_time = time_count();
    /** Wait Ack*/
    do
    {
        ...
        if((reply_msg_type == EDP_MSG_TYPE_CONN_RESP) && (ERR_TIMEOUT != ret))
            break;
...
static int32_t edp_send_ping(struct iot_edp_t *dev)
{
    ...
    if(send_len != tcp_send(dev->conn_fd, dev->send_buf, send_len, 0))
        return ERR_NETWORK;
...
static int32_t edp_push_data(struct iot_edp_t *dev, const int8_t *target, uint8_t *data, uint32_t data_len,
                             handle_t cd_tmr)
{
...
int32_t iot_edp_push_rawdata(void *dev, const int8_t *target, void *raw_data, uint32_t data_len, uint32_t timeout_ms)
{
    handle_t cd = countdown_start(timeout_ms);
    int32_t ret = ERR_OK;
