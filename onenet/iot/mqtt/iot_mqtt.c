...
/*****************************************************************************/
static void iot_mqtt_message_arrived(void *arg, const int8_t *topic, struct mqtt_message_t *message)
{
    struct iot_mqtt_t *mqtt_dev = (struct iot_mqtt_t *)arg;
    uint8_t *tmp_ptr = NULL;
    struct iot_dev_cmd_t dev_cmd = {0};

    if(NULL != (tmp_ptr = osl_strstr(topic, "$creq/")))
    {
        ...
        if(ERR_OK == mqtt_dev->evt_cb(IOT_DEV_CMD_RECEIVED, &dev_cmd, sizeof(dev_cmd)))
        {
            ...
            osl_memcpy(mqtt_dev->cmd_resp_topic, "$crsp", 5);
        }
        else
        {
...
void *iot_mqtt_open(struct iot_cloud_config_t *cloud, iot_dev_event_cb event_cb, uint32_t timeout_ms)
{
    ...
    if(mqtt_dev)
    {
        ...
        {
            mqtt_dev->client =
                mqtt_connect(IOT_MQTT_SERVER_ADDR, IOT_MQTT_SERVER_PORT, NULL, 0, &mqtt_param, timeout_ms);
        }
...
exit1:
    mqtt_disconnect(mqtt_dev, timeout_ms);
exit:
...
int32_t iot_mqtt_push_rawdata(void *dev, const int8_t *target, void *raw_data, uint32_t data_len, uint32_t timeout_ms)
{
    struct iot_mqtt_t *mqtt_dev = (struct iot_mqtt_t *)dev;
...
int32_t iot_mqtt_notify(void *dev, struct iot_data_t *data, uint32_t timeout_ms)
{
    ...
    ret = iot_mqtt_push_rawdata(mqtt_dev, IOT_MQTT_DS_TOPIC, raw_data, raw_data_len, timeout_ms);
    osl_free(raw_data);
    return ret;
}

int32_t iot_mqtt_observe(void *dev, const int8_t *resource, uint32_t timeout_ms)
{
    struct iot_mqtt_t *mqtt_dev = (struct iot_mqtt_t *)dev;
...
int32_t iot_mqtt_observe_cancel(void *dev, const int8_t *resource, uint32_t timeout_ms)
{
    struct iot_mqtt_t *mqtt_dev = (struct iot_mqtt_t *)dev;
