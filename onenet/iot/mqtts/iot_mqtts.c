...
static const int8_t mqtts_cert[] = {
    "-----BEGIN CERTIFICATE-----\r\n"
...
static uint32_t construct_payload(uint32_t msg_id, uint8_t *payload, struct iot_data_t *data)
{
    ...
    switch (data->data_type)
    {
        ...
        {
            cJSON_AddStringToObject(val_item, "v", data->buf.val_str);
            break;
        }
        default: break;
    }
    ...
    cJSON_AddItemToObject(dp_item, data->data_id, val_array);
    cJSON_AddItemToObject(root, "dp", dp_item);

    if (NULL != (temp_str = cJSON_PrintUnformatted(root)))
    {
...
static void iot_mqtts_message_arrived(void *arg, const int8_t *topic, struct mqtt_message_t *message)
{
    struct iot_mqtts_t *mqtts_dev = (struct iot_mqtts_t *)arg;
    struct iot_dev_cmd_t cmd_data = {0};
    uint8_t *tmp_ptr = NULL;
    uint16_t topic_len = osl_strlen(topic);

    if (NULL != (tmp_ptr = osl_strstr(topic, "$sys")))
    {
        if (NULL != (tmp_ptr = osl_strstr(topic, "/cmd/request/")))  //data cmd
        {
            ...
            if(ERR_OK == mqtts_dev->evt_cb(IOT_DEV_CMD_RECEIVED, &cmd_data, sizeof(struct iot_dev_cmd_t)))
            {
                ...
                osl_strcat(mqtts_dev->cmd_resp_topic, "response/");
        ...
        else if (NULL != (tmp_ptr = osl_strstr(topic, "/post/json/")))  //data notify reply
        {
            cJSON *reply = cJSON_ParseWithLength(message->payload, message->payload_len);
            if(osl_strstr(tmp_ptr, "accepted"))
            {
            ...
            mqtts_dev->reply_id = cJSON_GetObjectItem(reply, "id")->valueint;
            cJSON_Delete(reply);
...
static int8_t *iot_mqtts_topic_construct(const int8_t *format, const int8_t *pid, const int8_t *dev_name)
{
...
void *iot_mqtts_open(struct iot_cloud_config_t *cloud, iot_dev_event_cb event_cb, uint32_t timeout_ms)
{
    ...
    dev_token_generate(dev_token, SIG_METHOD_SHA1, 2524579200, 
                       cloud->product_id, 
                       cloud->device_sn, 
                       cloud->auth_code);
    log_debug("token = %s\n", dev_token);
    ...
    mqtt_param.connect_flag = MQTT_CONNECT_FLAG_CLEAN_SESSION | MQTT_CONNECT_FLAG_USERNAME | 
    ...
    mqtts_dev->topic.post = iot_mqtts_topic_construct(MQTTS_DEFAULT_TOPIC_POST, cloud->product_id, cloud->device_sn);
    mqtts_dev->topic.cmd_resp = iot_mqtts_topic_construct(MQTTS_DEFAULT_TOPIC_CMD_RESPONSE, cloud->product_id, cloud->device_sn);
    mqtts_dev->topic.subed = iot_mqtts_topic_construct(MQTTS_DEFAULT_SUBED_TOPIC, cloud->product_id, cloud->device_sn);
    ...
    {
#ifdef CONFIG_NETWORK_TLS
        mqtts_dev->client = mqtt_connect(IOT_MQTTS_SERVER_ADDR_TLS, IOT_MQTTS_SERVER_PORT_TLS, mqtts_cert,
                                         osl_strlen(mqtts_cert), &mqtt_param, timeout_ms);
#else
        mqtts_dev->client = mqtt_connect(IOT_MQTTS_SERVER_ADDR, IOT_MQTTS_SERVER_PORT, NULL, 0, &mqtt_param, timeout_ms);
#endif
    }
    ...
    if (0 != mqtt_subscribe(mqtts_dev->client, mqtts_dev->topic.subed, MQTT_QOS0, iot_mqtts_message_arrived,
                            mqtts_dev, countdown_left(cd_hdl)))
    {
        goto exit3;
    }
...
int32_t iot_mqtts_push_rawdata(void *dev, const int8_t *target, void *raw_data, uint32_t data_len, uint32_t timeout_ms)
{
    struct iot_mqtts_t *mqtts_dev = (struct iot_mqtts_t *)dev;
...
int32_t iot_mqtts_observe(void *dev, const int8_t *resource, uint32_t timeout_ms)
{
    struct iot_mqtts_t *mqtts_dev = (struct iot_mqtts_t *)dev;
...
int32_t iot_mqtts_observe_cancel(void *dev, const int8_t *resource, uint32_t timeout_ms)
{
    struct iot_mqtts_t *mqtts_dev = (struct iot_mqtts_t *)dev;
