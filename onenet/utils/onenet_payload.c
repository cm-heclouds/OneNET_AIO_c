...
/*****************************************************************************/
static int8_t* ts2str(uint32_t timestamp)
{
    struct tm *tm = NULL;
    time_t tmp = timestamp;

    tm = localtime(&tmp);
    osl_sprintf(ts_str, "%d-%02d-%02d %02d:%02d:%02d", tm->tm_year + 1900, tm->tm_mon + 1, tm->tm_mday, tm->tm_hour,
            tm->tm_min, tm->tm_sec);
    return ts_str;
}
...
static uint32_t construct_type2(uint8_t *payload, struct iot_data_t *data)
{
    ...
    if(data->timestamp)
    {
        ret = osl_sprintf(payload + offset + 2, ONENET_DATA_TYPE2_DESC_WITH_TIME_FMT,
                          data->data_id, ts2str(data->timestamp));
    }
    else
    {
        ret = osl_sprintf(payload + offset + 2, ONENET_DATA_TYPE2_DESC_FMT, data->data_id);
    }
...
static uint32_t construct_type3(uint8_t *payload, struct iot_data_t *data)
{
    ...
    payload[offset++] = 3;
    osl_strcpy(fmt, ONENET_DATA_TYPE3_NUM_FMT);
    ...
    {
        ret = osl_sprintf(payload + offset + 2, ONENET_DATA_TYPE3_STR_FMT, data->data_id,
                          data->buf.val_str);
    }
    offset += set_16bit_be(payload + offset, ret);
...
static uint32_t construct_type4(uint8_t *payload, struct iot_data_t *data)
{
    ...
    payload[offset++] = 4;
    osl_strcpy(fmt, ONENET_DATA_TYPE4_NUM_FMT);

    if(data->data_type == IOT_DATA_TYPE_INT)
    {
        osl_memcpy(fmt + 13, "llu", 3);
        ret = osl_sprintf(payload + offset + 2, fmt, data->data_id, ts2str(data->timestamp),
                          data->buf.val_int);
    }
    else if(data->data_type = IOT_DATA_TYPE_FLOAT)
    {
    ...
    {
        ret = osl_sprintf(payload + offset + 2, ONENET_DATA_TYPE4_STR_FMT, data->data_id, ts2str(data->timestamp),
                          data->buf.val_str);
    }
    offset += set_16bit_be(payload + offset, ret);
