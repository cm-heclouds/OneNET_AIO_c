...
/*****************************************************************************/
static void tm_mqtt_message_arrived(void *arg, const uint8_t *topic, struct mqtt_message_t *message)
{
    tm_obj->recv_cb(topic, message->payload, message->payload_len);
}

static uint8_t *tm_topic_construct(const uint8_t *format, const uint8_t *pid, const uint8_t *dev_name)
{
...
int32_t tm_mqtt_init(int32_t (*recv_cb)(const uint8_t * /** data_name*/, uint8_t * /** data*/, uint32_t /** data_len*/))
{
    ...
    tm_obj->mqtt_param.send_buf_len = CONFIG_SEND_BUF_LEN_MAX;
    ...
    tm_obj->mqtt_param.recv_buf_len = CONFIG_RECV_BUF_LEN_MAX;
...
int32_t tm_mqtt_login(const uint8_t *product_id, const uint8_t *dev_name, const uint8_t *dev_token, uint32_t life_time,
                      uint32_t timeout_ms)
{
    handle_t cd_hdl = 0;
    ...
    tm_obj->subed_topic = tm_topic_construct((const uint8_t *)THING_MODEL_SUBED_TOPIC, product_id, dev_name);
#ifdef CONFIG_GENERAL_OTA
    tm_obj->subed_ota_topic = tm_topic_construct((const uint8_t *)THING_MODEL_SUBED_OTA_TOPIC, product_id, dev_name);
#endif

#ifdef CONFIG_TM_MQTT_TLS
    if (NULL
        == (tm_obj->client = mqtt_connect((const uint8_t *)THING_MODEL_SERVER_ADDR_TLS, THING_MODEL_SERVER_PORT_TLS,
                                          tm_cert, osl_strlen(tm_cert), &(tm_obj->mqtt_param), countdown_left(cd_hdl))))
#else
    if (NULL
        == (tm_obj->client = mqtt_connect((const uint8_t *)THING_MODEL_SERVER_ADDR, THING_MODEL_SERVER_PORT, NULL, 0,
                                          &(tm_obj->mqtt_param), countdown_left(cd_hdl))))
#endif
    {
        goto exit;
    }
    ...
    if (0
        != (mqtt_subscribe(tm_obj->client, tm_obj->subed_topic, MQTT_QOS0, tm_mqtt_message_arrived, NULL,
...
#ifdef CONFIG_GENERAL_OTA
    if (0
        != (mqtt_subscribe(tm_obj->client, tm_obj->subed_ota_topic, MQTT_QOS0, tm_mqtt_message_arrived, NULL,
                           countdown_left(cd_hdl))))
    {
        goto exit;
    }
