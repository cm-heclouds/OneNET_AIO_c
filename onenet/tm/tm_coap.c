...
struct tm_coap_ctx_t
{
    ...
    handle_t net_socket;
    int32_t (*recv_cb)(const uint8_t * /** data_name*/, uint8_t * /** data*/, uint32_t /** data_len*/);
    uint8_t *recv_buf;
    uint16_t recv_mid;
...
static int32_t tm_coap_parse(const uint8_t *packet, uint32_t packet_len, uint8_t **get_uri_path, uint8_t **get_payload,
                             uint16_t *get_payload_len, uint8_t *get_code);
static int32_t tm_coap_send_post_packet(const uint8_t *uri, uint8_t *payload, uint32_t payload_len,
                                        uint8_t **get_uri_path, uint8_t **get_payload, uint16_t *get_payload_len,
...
static int32_t uri_add_path_tm_prefix(coap_packet_t *pkt, const char *suffix, const uint8_t *product_id,
...
static int32_t tm_udp_packet_handle(uint8_t **get_uri_path, uint8_t **get_payload, uint16_t *get_payload_len,
                                    handle_t timeout_cd);
/*****************************************************************************/
/*****************************************************************************/
/* Local Variables                                                           */
...
/*****************************************************************************/
int32_t tm_coap_init(int32_t (*recv_cb)(const uint8_t * /** data_name*/, uint8_t * /** data*/, uint32_t /** data_len*/))
{
    ...
    if (NULL == (tm_coap_ctx.recv_buf = osl_calloc(1, CONFIG_RECV_BUF_LEN_MAX)))
    {
        return ERR_NO_MEM;
    }
...
int32_t tm_coap_deinit(void)
{
    if (tm_coap_ctx.recv_buf)
    {
...
int32_t tm_coap_login(const uint8_t *product_id, const uint8_t *dev_name, const uint8_t *dev_token, uint32_t life_time,
                      uint32_t timeout_ms)
{
    int32_t  ret = ERR_OTHERS;
    uint16_t mid = 0;

    if (INIT != tm_coap_ctx.ctx_state)
    {
        return ERR_UNINITIALIZED;
    }
    if (life_time < COAP_LIFETIME_MIN_S || life_time > COAP_LIFETIME_MAX_S)
    {
        return ERR_INVALID_PARAM;
    }

    tm_coap_ctx.net_socket =
        plat_udp_connect((const uint8_t *)THING_MODEL_COAP_SERVER_ADDR, THING_MODEL_COAP_SERVER_PORT);
    ...
    if (NULL == (tm_coap_ctx.saastoken = osl_calloc(1, osl_strlen(dev_token) + 1)))
    {
        ret = ERR_NO_MEM;
        goto exit;
    }
    osl_strcpy(tm_coap_ctx.saastoken, dev_token);
...
exit:
    plat_udp_disconnect(tm_coap_ctx.net_socket);
    return ret;
}
...
int32_t tm_coap_reply(const uint8_t *payload, uint32_t payload_len, uint32_t timeout_ms)
{
                                     ...
                                     uint32_t timeout_ms)
{
    ...
    // udp send
    ret = plat_udp_send(socket, packet_buffer, packet_buffer_len, countdown_left(timer_handle));
...
{
    //$sys/{pid}/{device-name}/
    uint16_t header_uri_temp_length = 8 + osl_strlen((const uint8_t *)suffix) + osl_strlen((const uint8_t *)product_id)
                                      + osl_strlen((const uint8_t *)dev_name); // 7+1
    uint8_t *header_uri_temp = osl_calloc(1, header_uri_temp_length);
    if (header_uri_temp == NULL)
    {
        return ERR_NO_MEM;
    }

    osl_sprintf(header_uri_temp, (const uint8_t *)"$sys/%s/%s/%s", product_id, dev_name, suffix);
...
static int32_t payload_add_lifetime_and_saastoken(coap_packet_t *pkt, uint32_t lifetime, const uint8_t *saastoken)
{
    ...
    if (NULL == (payload_temp = osl_calloc(1, payload_temp_length)))
    {
        return ERR_NO_MEM;
    }

    osl_sprintf(payload_temp, (const uint8_t *)"{\"st\":\"%s\"", saastoken);

    if (lifetime != 0)
    {
        osl_sprintf(payload_temp + osl_strlen((const uint8_t *)payload_temp), (const uint8_t *)",\"lt\":%d", lifetime);
    }
    osl_strcat(payload_temp, (const uint8_t *)"}");

    coap_set_header_content_type(pkt, APPLICATION_JSON);
    coap_set_payload(pkt, (const void *)payload_temp, osl_strlen(payload_temp));
    return ERR_OK;
}
...
int32_t tm_coap_send_packet(const uint8_t *uri, uint8_t *payload, uint32_t payload_len, uint32_t timeout_ms)
{
    if (uri)
    {
                                    ...
                                    handle_t timeout_cd)
{
    ...
    while (0 == countdown_is_expired(timeout_cd))
    {
        ...
        recv_len = plat_udp_recv(tm_coap_ctx.net_socket, tm_coap_ctx.recv_buf, CONFIG_RECV_BUF_LEN_MAX,
                                 countdown_left(timeout_cd));
        if (recv_len)
        {
            ...
            parse_ret = tm_coap_parse(tm_coap_ctx.recv_buf, recv_len, &_uri, &_payload, &_payload_len,
                                      (uint8_t *)&(tm_coap_ctx.recv_response_code));
            ...
            osl_memset(tm_coap_ctx.recv_buf, 0, CONFIG_RECV_BUF_LEN_MAX);
            return recv_len;
        }
    }
    return 0;
