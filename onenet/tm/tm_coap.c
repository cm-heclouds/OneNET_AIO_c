...
int32_t tm_coap_init(int32_t (*recv_cb)(const int8_t * /** data_name*/, uint8_t * /** data*/, uint32_t /** data_len*/))
{
    ...
    tm_coap_ctx.recv_buf = osl_malloc(CONFIG_RECV_BUF_LEN_MAX);
                                    ...
                                    handle_t timeout_cd)
{
    ...
    while(0 == countdown_is_expired(timeout_cd))
    {
        ...
        recv_len =
            udp_recv(tm_coap_ctx.net_socket, tm_coap_ctx.recv_buf, CONFIG_RECV_BUF_LEN_MAX, countdown_left(timeout_cd));
        if(recv_len)
        {
