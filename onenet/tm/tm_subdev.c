...
/*****************************************************************************/
static int32_t subdev_data_callback(const uint8_t *name, void *data, uint32_t data_len)
{
    ...
    else if (NULL != osl_strstr(name, TM_TOPIC_SUBDEV_SERVICE_INVOKE))
    {
        ...
        else
        {
            ...
            tm_send_response(TM_TOPIC_SUBDEV_SERVICE_INVOKE_REPLY, id, 100, 0, NULL, 0, CONFIG_DEFAULT_REQUEST_TIMEOUT);
        }
        if (output_data)
        {
...
static int32_t subdev_topo_add_delete(const uint8_t *action, const uint8_t *product_id, const uint8_t *dev_name,
...
static int32_t subdev_login_logout(const uint8_t *action, const uint8_t *product_id, const uint8_t *dev_name,
                                   uint32_t timeout_ms)
{
    cJSON *  data     = cJSON_CreateObject();
    uint8_t *raw_data = NULL;
