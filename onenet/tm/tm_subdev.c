...
static int32_t subdev_data_callback(const int8_t *name, void *data, uint32_t data_len)
{
    ...
    if(NULL != osl_strstr(name, TM_TOPIC_SUBDEV_TOPO_CHANGE))
    {
        ...
        if(ERR_OK == ret)
        {
            tm_send_response(TM_TOPIC_SUBDEV_TOPO_CHANGE_REPLY, id, 200, 0, NULL, 0, CONFIG_TM_REPLY_TIMEOUT);
        }
        else
        {
            tm_send_response(TM_TOPIC_SUBDEV_TOPO_CHANGE_REPLY, id, 100, 0, NULL, 0, CONFIG_TM_REPLY_TIMEOUT);
        }
    }
    else if(NULL != osl_strstr(name, TM_TOPIC_SUBDEV_TOPO_GET_REPLY))
    {
        ...
        if(ERR_OK == ret)
        {
            tm_send_response(TM_TOPIC_SUBDEV_TOPO_GET_REPLY_RESULT, id, 200, 0, NULL, 0, CONFIG_TM_REPLY_TIMEOUT);
        }
        else
        {
            tm_send_response(TM_TOPIC_SUBDEV_TOPO_GET_REPLY_RESULT, id, 100, 0, NULL, 0, CONFIG_TM_REPLY_TIMEOUT);
        }
    }
    else if(NULL != osl_strstr(name, TM_TOPIC_SUBDEV_SERVICE_INVOKE))
    {
        ...
        if(ERR_OK == ret)
        {
            tm_send_response(TM_TOPIC_SUBDEV_SERVICE_INVOKE_REPLY, id, 200, 0, service_data, osl_strlen(output_data),
                             CONFIG_TM_REPLY_TIMEOUT);
        }
        else
        {
            cJSON_Delete((cJSON *)service_data);
            tm_send_response(TM_TOPIC_SUBDEV_SERVICE_INVOKE_REPLY, id, 100, 0, NULL, 0, CONFIG_TM_REPLY_TIMEOUT);
        }
        if(output_data)
        {
    ...
    else if(NULL != osl_strstr(name, TM_TOPIC_SUBDEV_PROP_GET))
    {
        ...
        {
            tm_send_response(TM_TOPIC_SUBDEV_PROP_GET_REPLY, id, 100, 0, NULL, 0, CONFIG_TM_REPLY_TIMEOUT);
        }
        if(prop_data)
        {
            osl_free(prop_data);
    ...
    else if(NULL != osl_strstr(name, TM_TOPIC_SUBDEV_PROP_SET))
    {
        ...
        if(ERR_OK == ret)
        {
            tm_send_response(TM_TOPIC_SUBDEV_PROP_SET_REPLY, id, 200, 1, NULL, 0, CONFIG_TM_REPLY_TIMEOUT);
        }
        else
        {
            tm_send_response(TM_TOPIC_SUBDEV_PROP_GET_REPLY, id, 100, 0, NULL, 0, CONFIG_TM_REPLY_TIMEOUT);
