                         ...
                         uint32_t resp_data_len, uint32_t timeout_ms)
{
    ...
    tm_mqtt_send_packet(topic, payload, payload_len, CONFIG_TM_REPLY_TIMEOUT);
    osl_free(topic);
#elif defined(CONFIG_TM_COAP)
    tm_coap_send_packet(NULL, payload, payload_len, CONFIG_TM_REPLY_TIMEOUT);
#endif
...
static void tm_prop_set(uint8_t *payload, uint32_t payload_len)
{
    ...
    if(NULL != props_data)
    {
        ...
        {
            tm_send_response(TM_TOPIC_PROP_SET_REPLY, id, 200, 0, NULL, 0, CONFIG_TM_REPLY_TIMEOUT);
        }
        else
        {
            tm_send_response(TM_TOPIC_PROP_SET_REPLY, id, 100, 0, NULL, 0, CONFIG_TM_REPLY_TIMEOUT);
        }
...
static void tm_prop_get(uint8_t *payload, uint32_t payload_len)
{
    ...
    if(NULL != props_data)
    {
        ...
        tm_send_response(TM_TOPIC_PROP_GET_REPLY, id, 200, 0, reply_data, 0, CONFIG_TM_REPLY_TIMEOUT);
...
#endif
                tm_send_response(topic, id, 200, 0, reply_data, 0, CONFIG_TM_REPLY_TIMEOUT);
#if defined(CONFIG_TM_MQTT)
                osl_free(topic);
#endif
            }
...
{
    const int8_t *action = res_name + osl_strlen(g_tm_obj.topic_prefix);

    if(0 == osl_strncmp(action, TM_TOPIC_PROP_SET, osl_strlen(TM_TOPIC_PROP_SET)))
    {
        tm_prop_set(payload, payload_len);
    }
    else if(0 == osl_strncmp(action, TM_TOPIC_PROP_GET, osl_strlen(TM_TOPIC_PROP_GET)))
    {
        tm_prop_get(payload, payload_len);
    }
    else if((0 == osl_strncmp(action, TM_TOPIC_PROP_POST, osl_strlen(TM_TOPIC_PROP_POST)))
            || (0 == osl_strncmp(action, TM_TOPIC_EVENT_POST, osl_strlen(TM_TOPIC_EVENT_POST)))
            || (0 == osl_strncmp(action, TM_TOPIC_DESIRED_PROPS_GET, osl_strlen(TM_TOPIC_DESIRED_PROPS_GET)))
            || (0 == osl_strncmp(action, TM_TOPIC_DESIRED_PROPS_DELETE, osl_strlen(TM_TOPIC_DESIRED_PROPS_DELETE)))
            || (0 == osl_strncmp(action, TM_TOPIC_PACK_DATA_POST, osl_strlen(TM_TOPIC_PACK_DATA_POST)))
            || (0 == osl_strncmp(action, TM_TOPIC_HISTORY_DATA_POST, osl_strlen(TM_TOPIC_HISTORY_DATA_POST))))
    {
        tm_post_reply(payload, payload_len);
    }
    else if(0 == osl_strncmp(action, "/service/", 9))
    {
        int8_t svc_id[32] = {0};
        int8_t *tmp_ptr = NULL;
        ...
        printf("Service Invoke [%s]\n", svc_id);

        tm_service_invoke(svc_id, payload, payload_len);
    }
#ifdef CONFIG_TM_GATEWAY
    else if(0 == osl_strncmp(action, "/sub/", 5))
    {
        log_debug("");
