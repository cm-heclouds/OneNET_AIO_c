...
int32_t http_add_header(void *ctx, const uint8_t *name, const uint8_t *value)
{
    ...
    if (PACK_HEADER != http_ctx->status)
    {
        ...
        {
            osl_sprintf((uint8_t *)(http_ctx->packet + osl_strlen((const uint8_t *)http_ctx->packet)),
                        (const uint8_t *)":%d", http_ctx->addr.port);
        }
        osl_strcat((uint8_t *)http_ctx->packet, (const uint8_t *)"\r\n");
    }
    osl_strcat((uint8_t *)http_ctx->packet, name);
...
static int32_t send_request(void *ctx, const uint8_t *server, uint16_t server_port, const uint8_t *ca_cert,
                            uint32_t ca_cert_len, uint8_t **resp_body, uint32_t *resp_body_len, uint32_t timeout_ms)
{
    struct http_ctx_t *       http_ctx = (struct http_ctx_t *)ctx;
    handle_t                  net_hdl  = -1;
...
#else
        ret = plat_tcp_send(net_hdl, http_ctx->packet + data_len, http_ctx->packet_len - data_len,
...
int32_t http_send_request(void *ctx, const uint8_t *server, uint16_t server_port, uint8_t **resp_body,
...
int32_t https_send_request(void *ctx, const uint8_t *server, uint16_t server_port, const uint8_t *ca_cert,
                           uint32_t ca_cert_len, uint8_t **resp_body, uint32_t *resp_body_len, uint32_t timeout_ms)
{
