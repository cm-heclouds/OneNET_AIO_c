...
EXPORT_SRCS_DIR := $(OUTPUT_DIR)/export
...
build-binary: $(obj-files)
	$(CC) -o onenet_aio $^ $(LIBS)

export: export-srcs
	@mkdir -p $(EXPORT_SRCS_DIR)/include
	@for inc in $(inc-y);								\
	do 													\
		hdrs=`find $$inc -maxdepth 1 -name "*.h"`;							\
		if [ -n "$$hdrs" ];								\
		then											\
			cp $$inc/*.h $(EXPORT_SRCS_DIR)/include/;	\
		fi 												\
	done
	@for inc in $(inc-sub-y);							\
		do cp -r $$inc $(EXPORT_SRCS_DIR)/include/;		\
	done
