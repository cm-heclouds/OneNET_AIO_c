...
struct uart_t
{
    ...
    uint32_t recv_thread_running;
    uart_recv_cb callback;
};
...
void* uart_recv_thread(void *arg)
{
    ...
    while(g_uart.recv_thread_running)
    {
        recv_len = uart_recv(g_uart.fd, recv_buf, 128, 1000);
        if(0 < recv_len)
        {
            g_uart.callback(recv_buf, recv_len);
        }
    }
...
handle_t uart_open(struct uart_config_t *port_config, uart_recv_cb callback)
{
    int32_t fd = -1;
    int32_t flags = 0;
    struct termios tios;
    ...
    if(callback)
    {
        ...
        g_uart.recv_thread_running = 1;
        pthread_create(&g_uart.recv_thread, NULL, uart_recv_thread, NULL);
    }
    return (handle_t)fd;
...
{
    if(g_uart.recv_thread_running)
    {
        void *retval = NULL;
        g_uart.recv_thread_running = 0;
        pthread_join(g_uart.recv_thread, &retval);
    }
    close((int32_t)uart);
}
